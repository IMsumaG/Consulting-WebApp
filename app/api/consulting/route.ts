...
export async function GET() {
  try {
    await requireAdmin();
    const requests = await prisma.consultationRequest.findMany({ orderBy: { createdAt: "desc" } });
    return ok({ requests });
  } catch (error) {
    if (error instanceof ApiAuthError) return fail(error.message, 401);
    return fail("Unable to load consulting requests", 500);
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const limit = checkRateLimit(`consulting:${ip}`, 4, 60_000);
    if (!limit.allowed) return fail("Please wait before submitting again.", 429);
    const body = await parseJson(request, schema);
    const requestRow = await prisma.consultationRequest.create({
      data: {
        clientName: body.name,
        organization: null,
        email: body.email,
        phone: body.phone || "Not provided",
        consultingType: body.subject,
    ...
    return fail("Unable to submit consulting request", 500);
  }
}
